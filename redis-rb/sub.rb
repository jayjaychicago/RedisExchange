...
def publish_smallest_depth(object)
	...
	if minscore.count != 0
		...
		$redis2.publish('SimplePriceFeed:' + object, '{"obj":"' + object +'","msgType":"S","P":"' +  minscore[1].to_f.abs.to_s + '","V":"' + qty.to_s + '"}')
	...
	if price < 0 then
		...
		if lastSmallestPrice != nil then
			...
			if price.abs < lastSmallestPrice.to_f.abs then
				...
				$redis2.publish('SimplePriceFeed:' + object, '{"obj":"' + object +'","msgType":"S","P":"' + price.abs.to_s + '","V":"' + final_depth.to_s + '"}' )
				$redis2.set(object + ':SMALLESTASKVOL', final_depth)
				$redis2.publish('SimpleQtyFeed:' + object, '{"obj":"' + object +'","msgType":"QS","V":"' + final_depth.to_s + '"}')
			end
		else
  ...
  if (pattern == 'PrivateMessageIN:*')
	...
	minscore = $redis2.get(data['objectId'] + ':SMALLESTASK').to_f
		...
		maxscore = $redis2.get(data['objectId'] + ':SMALLESTASKVOL').to_i
      		$redis2.publish('PrivateMessageOUT:' + event[17..-1].to_s, '{"obj":"' + data['objectId'] +'","msgType":"S","P":"' + minscore.abs.to_s + '","V":"' + maxscore.abs.to_s + '"}' )
		...
		minscore = $redis2.get(data['objectId'] + ':HIGHESTBIDVOL').to_i
                $redis2.publish('PrivateMessageOUT:' + event[17..-1],'{"obj":"' + data['objectId'] +'","msgType":"B","P":"' + maxscore.abs.to_s + '","V":"' + minscore.abs.to_s + '"}')
                #$redis2.publish('SimplePriceFeed:' + object, minscore[1].to_f.abs.to_s)
        else
