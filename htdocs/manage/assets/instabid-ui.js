...
function instabid_internal_feedback(msg) {
	...
	else if (buyNowThere == 1 && jmsg.msgType == "S") {
		updateBuyNow(jmsg);
	}
}
...
function populatePlaceBid(product,price,qty, side) {
	if ($("#price" + product).length && $("#qty" + product).length) {
		$("#price" + product).val(price);
		$("#qty" + product).val(qty);
		if (side == "S") {
			$("#placeBidBuy" + product).click();
			//$("#buy").attr('checked', true);
		}
		else {
			$("#placeBidSell" + product).click();
...
function populatePlaceSimpleBid(product,price,qty, side) {
	if ($("#price" + product + "S").length && $("#qty" + product + "S").length) {
		$("#price" + product + "S").val(price);
		$("#qty" + product+ "S").val(qty);
	...
	var products = getProducts(exchange, function(msg) {
			var jmsg = JSON.parse(msg);
                        ...
                        while (i<jmsg.PRODUCTS.length) {
                                finalTable = finalTable + "<TR id='instabid_PL" + jmsg.PRODUCTS[i].product + "'><TD id='instabid_P" + jmsg.PRODUCTS[i].product + "'><a href='/lumino/sbc.php?exchange=" + exchange + "&product=" + jmsg.PRODUCTS[i].product + "'>" + jmsg.PRODUCTS[i].product + "&nbsp;<span class='glyphicon glyphicon-pencil' aria-hidden='true'></span></a></TD><TD>-</TD><TD>-</TD><TD>-</TD><TD><div id='placeBid" + jmsg.PRODUCTS[i].product + "'></div></TD></TR>";
		...
		depthTableThere = 1;
			...
			console.log(msg);
				...
				bidList.push([parseFloat(jmsg.SELLDEPTHS[i].PRICE), jmsg.SELLDEPTHS[i].NUM]);
                                finalTable = finalTable + "<TR id='instabid_L" + jmsg.SELLDEPTHS[i].NUM + "' onclick='populatePlaceBid(\"" + product + "\"," + Math.abs(jmsg.SELLDEPTHS[i].PRICE) + "," + jmsg.SELLDEPTHS[i].QTY + ",\"S\")'><TD></TD><TD></TD><TD id='instabid_P" + jmsg.SELLDEPTHS[i].NUM + "'>$" + Math.abs(jmsg.SELLDEPTHS[i].PRICE) + "</TD><TD id='instabid_Q" + jmsg.SELLDEPTHS[i].NUM + "'>" + jmsg.SELLDEPTHS[i].QTY + "</TD></TR>";
//                                console.log(jmsg.SELLDEPTHS[i].QTY + '/' + Math.abs(jmsg.SELLDEPTHS[i].PRICE));
                                i = i + 1;
                        }
                        var i = 0;
				...
				bidList.push([parseFloat(jmsg.BUYDEPTHS[i].PRICE), jmsg.BUYDEPTHS[i].NUM]);
                                finalTable = finalTable + "<TR id='instabid_L" + jmsg.BUYDEPTHS[i].NUM + "' onclick='populatePlaceBid(\""  + product + "\"," + Math.abs(jmsg.BUYDEPTHS[i].PRICE) + "," + jmsg.BUYDEPTHS[i].QTY + ",\"B\")'><TD id='instabid_Q" + jmsg.BUYDEPTHS[i].NUM + "'>" + jmsg.BUYDEPTHS[i].QTY + "</TD><TD id='instabid_P" + jmsg.BUYDEPTHS[i].NUM + "'>$" + jmsg.BUYDEPTHS[i].PRICE + "</TD><TD></TD><TD></TD></TR>";
...
//                                console.log(jmsg.SELLDEPTHS[i].QTY + '/' + Math.abs(jmsg.SELLDEPTHS[i].PRICE));
				...
				simpleBidList.push([parseFloat(jmsg.BUYDEPTHS[i].PRICE), jmsg.BUYDEPTHS[i].NUM]);
                                finalTable = finalTable + "<TR id='instabid_SL" + jmsg.BUYDEPTHS[i].NUM + "' onclick='populatePlaceSimpleBid(\""  + product + "\"," + Math.abs(jmsg.BUYDEPTHS[i].PRICE) + "," + jmsg.BUYDEPTHS[i].QTY + ",\"B\")'><TD id='instabid_SQ" + jmsg.BUYDEPTHS[i].NUM + "'>" + jmsg.BUYDEPTHS[i].QTY + "</TD><TD id='instabid_SP" + jmsg.BUYDEPTHS[i].NUM + "'>$" + jmsg.BUYDEPTHS[i].PRICE + "</TD></TR>";
//                                console.log(jmsg.BUYDEPTHS[i].QTY + '/' + jmsg.BUYDEPTHS[i].PRICE);
                                i = i + 1;
                        }
                });     
			...
			console.log(msg);
				...
				if ((qtyO - filledO) != 0) {
					cancelText = "Cancel " + (qtyO - filledO) + " left" ;
                                ...
                                finalTable = finalTable + "<TR id='instabid_OL" + jmsg.ORDERS[i].NUM + "' onclick='populatePlaceBid(\""  + product + "\"," + Math.abs(jmsg.ORDERS[i].PRICE) + "," + jmsg.ORDERS[i].QTY + ",\"S\")'><TD>" + sideO + "</TD><TD id='instabid_OQ" + jmsg.ORDERS[i].NUM + "'>" + jmsg.ORDERS[i].QTY + "</TD><TD id='instabid_OF" + jmsg.ORDERS[i].NUM + "'>" + jmsg.ORDERS[i].QTYFILLED + "</TD><TD>" + product  + "</TD><TD id='instabid_OP" + jmsg.ORDERS[i].NUM + "'>$" + Math.abs(jmsg.ORDERS[i].PRICE) + "</TD><TD id='instabid_OT" + jmsg.ORDERS[i].NUM + "'>20" + timestampO.substr(0,6) + " " + timestampO.substr(6,2) + ':' + timestampO.substr(8,2) + ":" + timestampO.substr(10,2) + "</TD><TD><button>" + cancelText +  " </button></TD></TR>";
//                                console.log(jmsg.SELLDEPTHS[i].QTY + '/' + Math.abs(jmsg.SELLDEPTHS[i].PRICE));
                                i = i + 1;
                        }
                });
...
function buyNow(exchange, product, divToPlaceTable) {
	//divToPlaceTable.hide();
	var cc = 0;
	getProduct(exchange,product, function(msg) {
                var jmsg = JSON.parse(msg);
                if (jmsg["result"] == "SUCCESS") {
...
if (cc == 1) {
	...
	var depth = getDepth(exchange,product,1,function(msg) {
                        var jmsg = JSON.parse(msg);
                        var i = 0;
                        while (i<jmsg.SELLDEPTHS.length && i<1) {
				price = Math.abs(parseFloat(jmsg.SELLDEPTHS[i].PRICE));
				qty = Math.abs(parseFloat(jmsg.SELLDEPTHS[i].QTY));
                                i = i + 1;
                        }
	});

	table = table + "<div style='width:380px; '>";
table = table + '<div id="bidTool' + buyNowNumber + simpleBidAppend + '" ';
table = table + ' >';
table = table + '                        <form class="form-inline">';
if (simpleBid) {
	table = table + '               <input type="hidden" name="buySell' + buyNowNumber +  simpleBidAppend + '" id="placeBidBuy' + buyNowNumber +  simpleBidAppend + '" value="B" />';
}
else {
	table = table + '        <div class="btn-group" data-toggle="buttons">';
	table = table + '                <label class="btn btn-primary btn-sm">';
	table = table + '                        <input type="radio" name="buySell' + buyNowNumber +  simpleBidAppend + '" id="placeBidBuy' + buyNowNumber +  simpleBidAppend + '" value="B" autocomplete="off">Buy';
	table = table + '                </label>';
	table = table + '                <label class="btn btn-primary btn-sm">';
	table = table + '                        <input type="radio" name="buySell' + buyNowNumber +  simpleBidAppend + '" id="placeBidSell' + buyNowNumber +  simpleBidAppend +  '" value="S" autocomplete="off">Sell';
	table = table + '                </label>';
	table = table + '        </div>';
}
table = table + '        <div class="input-group input-group-sm">';
//table = table + '               <span class="input-group-addon">$</span>';
table = table + '               <input type="text" value="$' + price + '   (' + qty + ' left) " id="buyNowPriceDisplay' + buyNowNumber +  simpleBidAppend + '" style="text-align:right;border:none;max-width:200px;background-color:transparent;" class="form-control" readonly>';
table = table + '               <input type="hidden" value="' + price + '" id="buyNowPrice' + buyNowNumber +  simpleBidAppend + '" style="text-align:right;border:none;max-width:200px;background-color:transparent;" class="form-control" readonly>';
table = table + '               <input type="hidden" id="exchangeName' + buyNowNumber +  simpleBidAppend + '" name="exchangeName' + buyNowNumber +  simpleBidAppend + '" value="' + exchange + '" />';
table = table + '               <input type="hidden" id="productName' + buyNowNumber +  simpleBidAppend + '" name="productName' + buyNowNumber +  simpleBidAppend + '" value="' + product + '" />';
table = table + '        </div>';
table = table + '        <div class="input-group input-group-sm">';
table = table + '               <input type="number" id="buyNowQty' + buyNowNumber +  simpleBidAppend + '" style="max-width:100px;" class="form-control" placeholder="Qty">';
table = table + '        </div>';
table = table + '        <button type="submit" onclick="return false;" id="tradeButton' + buyNowNumber +  simpleBidAppend + '" class="btn btn-default btn-sm">Buy Now</button>';
table = table + '</form>';
table = table + '        </div>';
        table = table + "</div>";
divToPlaceTable.append(table);
...
$("#tradeButton" + buyNowNumber +  simpleBidAppend).click(function() {
		if (cc==1) {
			$(".stripe-button-el")[0].click(function() {
			});
		}
		if (simpleBid) {
			var buySell = $('input[name=buySell' + buyNowNumber +  simpleBidAppend + ']').val();
		} 
		else {
                	var buySell = $('input[name=buySell' + buyNowNumber +  simpleBidAppend + ']:checked').val();
		}
                //alert("You want to trade" + $("#exchangeName").val() + $("#productName").val() + buySell + $("#qty").val() + $("#price").val() + 'admin'  );
                if (buySell == "B" || buySell == "S") {
			var fills = newOrder($("#exchangeName" + buyNowNumber + simpleBidAppend ).val() , $("#productName" + buyNowNumber +  simpleBidAppend).val() , buySell , $("#buyNowQty"  + buyNowNumber +  simpleBidAppend).val() , $("#buyNowPrice" + buyNowNumber +  simpleBidAppend).val() , "admin", function(msg) { 
        	//alert(msg);
        	msgJson = JSON.parse(msg);
        	if (msgJson["fillCount"] == 0) { 
                	//$("#productDiv").append('<div class="alert alert-warning"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>Warning!</strong> ' + msg + '</div>');
                	$("#bidTool" + buyNowNumber +  simpleBidAppend).after('<div style="padding:10px; margin-top:10px;" id="alertPop" class="alert alert-warning"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>Bid successfully executed!</strong> However your bid was not immediately filled</div>');
			$("#alertPop").fadeOut(9999);
			$("#alertPop").hover(function() {
				$(this).stop().animate({opacity:'100'}); 
			});
                	...
                	for (var i = 0; i < msgJson.fills.length; i++) {
                        	var fill = msgJson.fills[i];
                        	$("#bidTool" + buyNowNumber +  simpleBidAppend).after('<div style="padding:10px; margin-top:10px;" id="alertPop" class="alert alert-warning"><a href="#" class="close" data-dismiss="alert">&times;</a><strong>Succesfully filled!</strong> Filled ' + fill.qty + ' at $' + fill.price + '</div>');
			 $("#alertPop").fadeOut(9999);
                        $("#alertPop").hover(function() {
                                $(this).stop().animate({opacity:'100'});
                        });
		...
		//buyNowNumberOverall = buyNowNumberOverall + 1;
		instabid_feed_highest_bid_and_lowest_sell(exchange, product)
}
...
function updateBuyNow(jsonMsg) {
	var postFix = "";
	if (jsonMsg.obj.search(":") == 0){
		postFix = jsonMsg.obj;
	}
	else {
		postFix = jsonMsg.obj.substring(jsonMsg.obj.search(":")+1);
	}
	$("#buyNowPrice" + postFix).val(parseFloat(jsonMsg.P));
	var vol = (typeof jsonMsg.V === "undefined") ? "nothing" : jsonMsg.V;
	$("#buyNowPriceDisplay" + postFix).val("$" + parseFloat(jsonMsg.P) + "    (" + vol + " left)");
	if (vol == "nothing") {
		$("#buyNow").hide();
	}
	else {
