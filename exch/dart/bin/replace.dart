...
main(List<String> args) async {
  ...
  RedisClient.connect('$host:$port').then((RedisClient redisClient) {
    final client = new ExchClient(redisClient);
    final req = new ReplaceReq(options['req-id'], options['user-id'],
        options['market-id'], options['order-id'], options['price'],
        options['quantity']);
    client.replace(req);
    redisClient.close();
  });
